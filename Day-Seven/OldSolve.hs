...
stepCost :: Step -> Int
stepCost step = fromMaybe 0 (lookup step (zip ['A'..'Z'] [1..])) - 1 + 60
...
-- singleStep ws rstate reqs = case notProcessed ws $ validOptions rstate reqs of
--     [] -> step ws rstate
--     xs -> step (fst $ allocateWorkers xs ws) rstate

--     where step ws rstate = case stepWorkers ws of
--                                 ([], nws) -> if all (==Nothing) nws 
...
-- stepThroughTimed :: [Maybe Work] -> RState -> [Requirement] -> RState 
-- stepThroughTimed ws rstate reqs = case validOptions rstate reqs of
--     [] -> if all (==Nothing) ws then rstate
--           else (\(nws, rstate) -> stepThroughTimed nws rstate reqs) (singleStep ws rstate reqs)
--     xs -> (\(nws, rstate) -> stepThroughTimed nws rstate reqs) (singleStep ws rstate reqs)

stepThroughTimed' :: [Maybe Work] -> RState -> [Requirement] -> [([Maybe Work], RState)] 
stepThroughTimed' ws rstate reqs = case validOptions rstate reqs of
    [] -> if all (==Nothing) ws then [(ws, rstate)]
            else (\(nws, rstate) -> (nws, rstate) : stepThroughTimed' nws rstate reqs) (singleStep' ws rstate reqs)
